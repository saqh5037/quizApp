...
# =====================================================
# AristoTest Complete QA Deployment Script
# Version: 2.0.0
# Date: 2024-08-31
# Description: Deploy latest AristoTest from main branch to QA server
# =====================================================
...
DB_NAME="aristotest2"  # Important: NOT aristotest
DB_USER="aristotest"
...
check_ssh() {
    ...
    ssh -i "$SSH_KEY" -o ConnectTimeout=5 "$EC2_USER@$EC2_HOST" "echo 'SSH connection successful'" &>/dev/null
    if [ $? -eq 0 ]; then
...
echo -e "${YELLOW}Step 2: Creating deployment script on remote server...${NC}"

cat << 'DEPLOY_SCRIPT' | ssh -i "$SSH_KEY" "$EC2_USER@$EC2_HOST" "cat > /tmp/deploy_aristotest.sh"
#!/bin/bash
set -e
...
sudo chown -R ec2-user:ec2-user "$BACKEND_DIR" "$FRONTEND_DIR"
...
npm run build || {
    echo -e "${YELLOW}Build failed, using existing dist folder if available${NC}"
}
...
# Execute deployment
echo -e "${YELLOW}Step 3: Executing deployment on remote server...${NC}"
remote_exec "chmod +x /tmp/deploy_aristotest.sh && sudo /tmp/deploy_aristotest.sh"
...
echo "pm2 logs aristotest-qa-backend"
echo ""
